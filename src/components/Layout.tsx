...
const Layout = ({ children }: LayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-slate-50 flex">
      ...
      {/* Mobile Header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 glass z-40 border-b border-white/20">
      ...
      >
        ...
        <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
          ...
          <p className="px-4 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">إعدادات</p>
          <Link to="/settings">
            <Button
              variant="ghost"
              className="w-full justify-start gap-3 h-12 rounded-xl text-slate-600 hover:bg-slate-100/50 hover:text-slate-800 transition-all duration-200"
            >
        ...
        <div className="p-3 border-t border-slate-100/80">
          <Button
            variant="ghost"
            className="w-full justify-start gap-3 h-12 rounded-xl text-slate-600 hover:bg-rose-50 hover:text-rose-600 transition-all duration-200"
          >
            <div className="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center text-rose-500 group-hover:bg-rose-500 group-hover:text-white transition-all">
