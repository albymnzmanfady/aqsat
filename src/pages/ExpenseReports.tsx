...
import { initialExpenses, initialExpenseCategories, initialInstallments } from "@/data/mockData";
...
const ExpenseReports = () => {
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const monthRevenue = installments
        .filter((inst) => {
          if (!inst.isPaid || !inst.paidDate) return false;
  ...
  const totalRevenue = installments.filter((i) => i.isPaid).reduce((s, i) => s + i.amount, 0);
  const netProfit = totalRevenue - totalExpenses;
  const profitMargin = totalRevenue > 0 ? Math.round((netProfit / totalRevenue) * 100) : 0;

  const yearExpenses = expenses.filter((e) => new Date(e.date).getFullYear() === currentYear);
  const yearRevenue = installments.filter((i) => i.isPaid && i.paidDate && new Date(i.paidDate).getFullYear() === currentYear);
  ...
  return (
    <Layout>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">تقارير المصروفات</h1>
          <p className="text-slate-500 mt-1">تحليل المصروفات ومقارنتها بالإيرادات</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-rose-600">{totalExpenses.toLocaleString()} ج.م</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-xl text-emerald-600">{totalRevenue.toLocaleString()} ج.م</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card className="border-0 bg-white/70 backdrop-blur-sm">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className={cn(
                "w-12 h-12 rounded-2xl shadow-md flex items-center justify-center",
                netProfit >= 0 ? "bg-gradient-to-br from-blue-500 to-cyan-500" : "bg-gradient-to-br from-amber-500 to-orange-500"
              )}>
                <Wallet className="h-6 w-6 text-white" />
              </div>
              <div>
                <p className="text-sm text-slate-500">صافي الربح</p>
                <p className={cn("font-bold text-xl", netProfit >= 0 ? "text-blue-600" : "text-amber-600")}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={cn("font-bold text-xl", profitMargin >= 0 ? "text-violet-600" : "text-rose-600")}>
                  {profitMargin}%
                </p>
              </div>
            </div>
      ...
      <Card className="mb-8 border-0 bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-600 text-white overflow-hidden relative">
        ...
        <CardContent className="p-6 relative z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="font-bold text-lg">ملخص العام {currentYear}</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
              <p className="text-sm text-white/80 mb-1">الإيرادات</p>
              <p className="text-2xl font-bold">{yearTotalRevenue.toLocaleString()} ج.م</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
              <p className="text-sm text-white/80 mb-1">المصروفات</p>
              <p className="text-2xl font-bold">{yearTotalExpense.toLocaleString()} ج.م</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
              <p className="text-sm text-white/80 mb-1">صافي الربح</p>
              <p className="text-2xl font-bold">{yearNet.toLocaleString()} ج.م</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="border-0 bg-white/70 backdrop-blur-sm overflow-hidden">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="font-bold text-slate-800">مقارنة شهرية (المصروفات vs الإيرادات)</h3>
            </div>
            <div className="h-72">
        ...
        <Card className="border-0 bg-white/70 backdrop-blur-sm overflow-hidden">
          <CardContent className="p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="font-bold text-slate-800">توزيع المصروفات حسب الفئة</h3>
            </div>
      ...
      <Card className="border-0 bg-white/70 backdrop-blur-sm overflow-hidden">
        <CardContent className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-slate-800">تفصيل المصروفات حسب الفئة</h3>
          </div>
              ...
              return (
                <div key={index} className="p-3 bg-slate-50/50 rounded-xl">
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                    ...
                    <span>{item.value.toLocaleString()} ج.م</span>
                  </div>
