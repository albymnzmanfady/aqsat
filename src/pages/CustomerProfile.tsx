...
const CustomerProfile = () => {
  ...
  const stats = useMemo(() => {
    ...
    if (overdueInstallments.length > 0) {
      const totalDays = overdueInstallments.reduce((sum, i) => {
        ...
        return sum + Math.floor((today.getTime() - due.getTime()) / 86400000);
      }, 0);
    ...
    const onTimePaid = customerInstallments.filter((i) => {
      ...
      return new Date(i.paid_date) <= new Date(i.year, i.month - 1, i.day);
    }).length;
    ...
    return {
      ...
      totalInstallments, paidInstallments, unpaidInstallments,
      totalValue, totalPaid, totalRemaining,
      paymentRate, totalOverdue, avgDelayDays,
      riskScore, riskLevel, onTimePaid, onTimeRate,
    };
  ...
  const handleSendWhatsApp = async () => {
    ...
    if (result.success) showSuccess("✅ تم إرسال رسالة الترحيب");
          ...
          <CardContent className="p-6 lg:p-8 relative z-10">
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              <div className="flex items-center gap-5">
                <div className="w-20 h-20 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-white font-bold text-3xl border border-white/30">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2">
                    <Badge className="bg-white/20 text-white border-white/30 rounded-lg backdrop-blur-sm">
                      <Phone className="h-3 w-3 ml-1" />{customer.phone}
                    </Badge>
                    <Badge className="bg-white/20 text-white border-white/30 rounded-lg backdrop-blur-sm">
              ...
              <div className="flex gap-3 md:mr-auto">
                ...
                <Button onClick={() => navigate("/contracts")} className="rounded-xl bg-white text-violet-700 hover:bg-white/90 gap-2 font-bold">
                  <FileText className="h-4 w-4" />عقد جديد
                </Button>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                <div className="flex items-center gap-2 text-white/70 text-xs mb-1"><PhoneCall className="h-3 w-3" />الهاتف</div>
                <p className="font-bold" dir="ltr">{customer.phone}</p>
              </div>
              {customer.national_id && (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                  <div className="flex items-center gap-2 text-white/70 text-xs mb-1"><Hash className="h-3 w-3" />الرقم القومي</div>
                  <p className="font-bold" dir="ltr">{customer.national_id}</p>
                </div>
              )}
              {customer.address && (
                <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                  <div className="flex items-center gap-2 text-white/70 text-xs mb-1"><MapPin className="h-3 w-3" />العنوان</div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-slate-800"><AnimatedCounter value={stats.totalContracts} /></p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{stats.activeContracts} نشط • {stats.completedContracts} مكتمل</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center"><FileText className="h-6 w-6 text-white" /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 bg-white/70 backdrop-blur-sm hover-lift">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-emerald-600"><AnimatedCounter value={stats.totalPaid} formatter={(v) => v.toLocaleString()} /><span className="text-xs font-medium text-slate-400 mr-1">ج.م</span></p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{stats.paidInstallments} قسط مدفوع</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center"><Wallet className="h-6 w-6 text-white" /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 bg-white/70 backdrop-blur-sm hover-lift">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-amber-600"><AnimatedCounter value={stats.totalRemaining} formatter={(v) => v.toLocaleString()} /><span className="text-xs font-medium text-slate-400 mr-1">ج.م</span></p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{stats.unpaidInstallments} قسط باقي</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center"><Clock className="h-6 w-6 text-white" /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 bg-white/70 backdrop-blur-sm hover-lift">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-violet-600"><AnimatedCounter value={stats.paymentRate} /><span className="text-xs font-medium text-slate-400 mr-1">%</span></p>
                  <p className="text-[10px] text-slate-400 mt-0.5">{stats.onTimeRate}% في الموعد</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center"><TrendingUp className="h-6 w-6 text-white" /></div>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className={cn("border-2 overflow-hidden", risk.borderColor)}>
            <CardContent className="p-6">
              <div className="text-center">
                <div className={cn("w-24 h-24 rounded-3xl bg-gradient-to-br flex items-center justify-center mx-auto mb-4", risk.color)}>
                ...
                <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                  <div className={cn("h-full rounded-full bg-gradient-to-r transition-all duration-1000", risk.color)} style={{ width: `${stats.riskScore}%` }} />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 bg-white/70 backdrop-blur-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-emerald-50/50 rounded-xl text-center"><p className="text-lg font-extrabold text-emerald-600">{stats.paidInstallments}</p><p className="text-[10px] text-slate-500">قسط مدفوع</p></div>
                <div className="p-3 bg-amber-50/50 rounded-xl text-center"><p className="text-lg font-extrabold text-amber-600">{stats.unpaidInstallments}</p><p className="text-[10px] text-slate-500">قسط باقي</p></div>
                <div className="p-3 bg-rose-50/50 rounded-xl text-center"><p className="text-lg font-extrabold text-rose-600">{stats.totalOverdue}</p><p className="text-[10px] text-slate-500">قسط متأخر</p></div>
                <div className="p-3 bg-blue-50/50 rounded-xl text-center"><p className="text-lg font-extrabold text-blue-600">{stats.avgDelayDays}</p><p className="text-[10px] text-slate-500">متوسط أيام التأخير</p></div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                {customerContracts.map((contract) => {
                  ...
                  return (
                    <div key={contract.id} className="p-3 bg-slate-50/50 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => navigate("/contracts")}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div><p className="text-sm font-bold text-slate-700">{contract.product_type}</p><p className="text-[10px] text-slate-400">{contract.total_price.toLocaleString()} ج.م</p></div>
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-violet-500 to-purple-600 rounded-full transition-all" style={{ width: `${progress}%` }} /></div>
            ...
            {customerInstallments.length === 0 ? (
              ...
              <div className="divide-y divide-slate-100/80">
                {customerInstallments.sort((a, b) => new Date(b.year, b.month - 1, b.day).getTime() - new Date(a.year, a.month - 1, a.day).getTime()).map((inst) => {
                  ...
                  return (
                    <div key={inst.id} className={cn("flex flex-col sm:flex-row sm:items-center justify-between p-4 gap-3 hover:bg-slate-50/50 transition-colors", isOverdue && "bg-rose-50/30 border-r-4 border-r-rose-400")}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                            ...
                            {contract && <span className="bg-slate-100 px-2 py-0.5 rounded-md">{contract.product_type}</span>}
                          ...
                          {inst.is_paid ? (
                            ...
                            <>
                              <Button variant="ghost" size="sm" className={cn("h-9 px-3 rounded-xl gap-1.5 font-bold text-xs active:scale-90", isOverdue ? "text-white bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600" : "text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600")} disabled={payingId === inst.id} onClick={() => contract && setConfirmPayDialog({ installment: inst, contract })}>
                                {payingId === inst.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4" />}
                                سداد
                              </Button>
        ...
        <DialogContent className="sm:max-w-[420px] rounded-3xl p-0 overflow-hidden">
          ...
          <DialogFooter className="px-6 pb-6 grid grid-cols-2 gap-3">
            ...
            <Button onClick={handlePayInstallment} disabled={payingId !== null} className="rounded-xl h-11 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-bold gap-2">
              {payingId ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4" />}
              نعم، تم الاستلام
            </Button>
          </DialogFooter>
        </DialogContent>
