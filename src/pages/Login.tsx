    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-slate-50 flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-md relative z-10 flex flex-col items-center">
        <div className="text-center mb-8">
          ...
          {settings.companyName && <p className="text-sm text-slate-400 mt-2 font-medium">{settings.companyName}</p>}
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="pr-12 rounded-2xl h-12 bg-white/50 border-slate-200/50 focus:border-violet-300" placeholder="admin@system.com" dir="ltr" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="pr-12 rounded-2xl h-12 bg-white/50 border-slate-200/50" placeholder="••••••••" dir="ltr" />
            ...
            <Button type="submit" disabled={loading} className="w-full h-12 rounded-2xl bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700 shadow-lg shadow-violet-500/30 text-base gap-2">
              {loading ? <div className="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <LogIn className="h-5 w-5" />}
              تسجيل الدخول
            </Button>
          </form>
        ...
        <div className="mt-8 text-center space-y-3">
          <div className="flex items-center justify-center gap-2 text-slate-400">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-md"><Code className="h-4 w-4 text-white" /></div>
          ...
          <div className="flex items-center justify-center gap-1 text-xs text-slate-400">
            <span>صُنع بـ</span><Heart className="h-3 w-3 text-rose-400 fill-rose-400" /><span>© {new Date().getFullYear()} {settings.appName || "أقساط"}</span>
          </div>
        </div>
      </div>
    </div>
